...
interface OrderEmailRequest {
  type: 'payment_submitted' | 'payment_confirmed' | 'payment_rejected' | 'order_delivered';
...
const getEmailTemplate = (data: OrderEmailRequest): { subject: string; html: string } => {
  const { type, customerName, orderNumber, orderTotal, orderDate, deliveryDate, items } = data;

  if (type === 'order_delivered') {
    const itemsHtml = items ? items.map(item => `
      <tr>
        <td style="padding: 12px 0; border-bottom: 1px solid #edf2f7; color: #4a5568; font-size: 14px;">${item.product_name} x ${item.quantity}</td>
        <td style="padding: 12px 0; border-bottom: 1px solid #edf2f7; color: #2d3748; font-size: 14px; font-weight: 600; text-align: right;">${Number(item.total_price).toFixed(3)} BHD</td>
      </tr>
    `).join('') : '';
                    ...
                    <tr>
                      <td style="padding: 40px 30px;">
                        ...
                        <p style="margin: 0 0 30px; color: #333333; font-size: 16px; line-height: 1.6;">
                          Your order has been successfully delivered! We hope you and your pets love the products.
                        </p>
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f0fdf4; border: 2px solid #10b981; border-radius: 8px; margin-bottom: 30px;">
                          <tr>
                            <td style="padding: 24px;">
                              <h2 style="margin: 0 0 16px; color: #10b981; font-size: 18px; font-weight: 600;">Delivery Summary</h2>
                              <table width="100%" cellpadding="8" cellspacing="0">
                                ...
                                <tr>
                                  <td style="color: #666666; font-size: 14px;">Delivery Date:</td>
                                  <td style="color: #333333; font-size: 14px; font-weight: 600; text-align: right;">${deliveryDate || new Date().toLocaleDateString()}</td>
                                </tr>
                                <tr>
                                  <td style="color: #666666; font-size: 14px;">Total Paid:</td>
                        ...
                        <!-- Small Invoice / Items Table -->
                        ${itemsHtml ? `
                        <h3 style="margin: 0 0 16px; color: #333333; font-size: 18px; font-weight: 600;">Purchased Items</h3>
                        <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 30px;">
                          <thead>
                            <tr>
                              <th align="left" style="padding-bottom: 12px; border-bottom: 2px solid #edf2f7; color: #718096; font-size: 12px; font-weight: 600; text-transform: uppercase;">Product</th>
                              <th align="right" style="padding-bottom: 12px; border-bottom: 2px solid #edf2f7; color: #718096; font-size: 12px; font-weight: 600; text-transform: uppercase;">Amount</th>
                            </tr>
                          </thead>
                          <tbody>
                            ${itemsHtml}
                            <tr>
                              <td style="padding-top: 20px; color: #2d3748; font-size: 16px; font-weight: 700;">Total</td>
                              <td align="right" style="padding-top: 20px; color: #10b981; font-size: 16px; font-weight: 700;">${orderTotal} BHD</td>
                            </tr>
                          </tbody>
                        </table>
                        ` : ''}
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0">
                          <tr>
                            <td align="center" style="padding: 20px 0;">
                              <a href="https://purrkinpetsbh.com/shop" style="display: inline-block; background: linear-gradient(135deg, #10b981 0%, #3b82f6 100%); color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-weight: 600; font-size: 16px;">Continue Shopping</a>
                            </td>
                          </tr>
                        </table>
                      </td>
